...
class SpongeSchematicV2(BaseModel, AbstractRegion, AbstractSchematic):
    Version: nbt.Int
    ...
    BlockEntities: nbt.List[Entity] = Field(default_factory=nbt.List)
    Entities: nbt.List[Entity] = Field(default_factory=nbt.List)
    BiomePaletteMax: nbt.Int = Field(default=nbt.Int(0))
    BiomePalette: nbt.Compound[nbt.Int] = Field(default_factory=nbt.Compound)
    # BiomePaletteData: nbt.ByteArray = Field(default_factory=nbt.ByteArray)
    ...
    @field_validator("BlockEntities", "Entities", mode="plain")
    @classmethod
    def validate_entities(cls, value):
        return nbt.List([Entity(v) for v in value])
    ...
    def get_tile_entity_matrix(self) -> dict[tuple[int, int, int], Entity]:
        return {entity.pos.astuple(): entity for entity in self.BlockEntities}
    ...
    def from_schematic(cls, schematic: AbstractSchematic, target_version: Optional["Version"]) -> Self:
        ...
        if target_version:
            ...
            source_version = schematic.get_minecraft_version()
        ...
        return cls.model_validate({
            ...
            "DataVersion": source_version.data_version,
